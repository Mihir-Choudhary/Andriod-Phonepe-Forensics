"""
PhonePe Android Forensics — Core primitives
==========================================
Android-specific parsing primitives. Most heavy lifting is REUSED from
``phonepe_forensics.core`` (SQLiteReader, normalize_timestamp, decode_txn_id,
amount_to_rupees, safe_int/safe_float, hash_file) — those are platform-neutral.

What's Android-specific and lives here:
  * ``AndroidCasePaths``  — resolve the app-data layout (databases/, shared_prefs/, files/,
    app_webview/) (the com.phonepe.app data layout).
  * ``decode_json_blob``  — Android payloads are plain JSON strings (the whole decoder).
  * ``first_or_dict``     — ZDATA-style fields swing between list and dict (e.g. RECEIVED `from`
    is a dict, SENT `to` is a list). Normalize to a single dict.
  * ``read_shared_pref``  — Android shared_prefs/*.xml reader (flatten to {key: value}).
"""
